...
router.post("/register", (req, res) => {
  ...
  var nameVar = req.body.username; // name คือตัวที่พิมลง postman     var passwordVar = req.body.password; // password พิมค่าลง postman
  MongoClient.connect(
    ...
    function(err, db) {
      ...
      let userObj = {
        ...
        username: nameVar // username คือค่า attribute ใน db                 password: passwordVar // password คือค่า attribute ใน db
      };
      dbo
        .collection("userLoginTable")
        .findOne({ username: nameVar }, function(err, result) {
          if (err) {
          ...
          if (result) {
            res.send({ status: false, message: "มี username นี้แล้ว" });
          } else {
            dbo
              .collection("userLoginTable")
              .insertOne(userObj, function(err, result) {
...
router.post("/login", (req, res) => {
  ...
  MongoClient.connect(
    ...
    function(err, db) {
      ...
      dbo
        .collection("userLoginTable")
        .findOne({ username: nameVar, password: passwordVar }, function(
          err,
          user
        ) {
          if (err) {
          ...
          if (user) {
            var jwtBearerToken = jwt.sign(
              ...
              "secret",
              {
                expiresIn: "365d" // 1 Year                         subject: "JWT"
              }
            );
            res.send(JSON.stringify({ status: true, token: jwtBearerToken }));
          } else {
            res.send({ status: false });
          }
        });
...
router.post("/verifyToken", (req, res) => {
  jwt.verify(req.body.token, "secret", {}, function(err, payload) {
    if (payload) {
      res.send(JSON.stringify({ verify: true }));
    } else {
      res.send(JSON.stringify({ verify: false }));
    }
  });
});
